...
function main(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "-h" || arg === "--help" || arg[0] !== "-") {
      ...
      if (i + 1 === args.length) {
        throw new Error(
          "Missing value for " + JSON.stringify(arg) + ' (use "--help" for usage)'
        );
      }
      flags[arg] = args[++i];
    } else {
  ...
  let compiled;
  try {
    parsed = typeof content === "string" ? parseSchema(content) : decodeBinarySchema(new ByteBuffer(content));
